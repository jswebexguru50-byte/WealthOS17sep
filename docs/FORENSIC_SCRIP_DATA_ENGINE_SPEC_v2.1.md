# Forensic Scrip Data Engine — Spec & Design v2.1
**Revision basis:** expanded FR3 field set, realistic token/quota math, GitHub open-source accelerators, quality-gate and failsafe hardening, and multi-exchange/listing-segment customization (BSE Main, NSE Main, BSE SME, NSE Emerge).
...
| FR9 | Apply platform-aware processing across all four listing categories — BSE Main Board, NSE Main Board, BSE SME, NSE Emerge — accounting for differing disclosure obligations, concall availability, and data coverage per segment |
...
**Platform awareness:** BSE Main, BSE SME, NSE Main, and NSE Emerge are polled as four distinct source endpoints, since announcement feeds and compliance calendars differ by segment even on the same exchange (see Section 12). Each scrip carries a `listingPlatform` tag, not an assumed "BSE or NSE" binary.
**Built from:** forked `fundamental-screener` (ELT + landing zone) + `screener-finance` (peers, shareholding, ratios) — verify actual SME/Emerge coverage of these accelerators before assuming parity with mainboard coverage.
...
class ForensicExtraction(BaseModel):
    ...
    scripCode: str
    listingPlatform: Literal["BSE_MAIN", "NSE_MAIN", "BSE_SME", "NSE_EMERGE"]
    quarter: str
...
**Segment coverage note:** the Stage 1–3 validation set should include at least a handful of BSE SME and NSE Emerge scrips alongside mainboard names, not mainboard-only — Tier 1 concall availability and Tier 0 disclosure completeness differ enough by segment (Section 12) that a mainboard-only pilot won't surface SME/Emerge-specific gaps until Stage 4, when they're more expensive to fix.
...
## 11. Multi-Exchange & Listing-Segment Customization

Treating "BSE" and "NSE" as two interchangeable sources misses that each hosts two structurally different segments — Main Board and SME (BSE SME, NSE Emerge) — with different regulatory obligations, disclosure completeness, and data availability. This section makes that explicit rather than leaving it implicit in source adapters.

### 11.1 Regulatory differences that affect data availability

- SME-listed entities are exempt from LODR Regulations 17–27 and 46(2)(b)-(i), (t), and Schedule V Paras C/D/E — the core corporate governance disclosure chapter — unless the company's paid-up equity capital exceeds ₹10 Cr or net worth exceeds ₹25 Cr, in which case it must comply within 6 months and remains compliant thereafter even if metrics later fall back below threshold.
- **Implication:** the absence of a governance disclosure category for an SME/Emerge scrip may be a genuine non-obligation, not a data-collection gap. `quality_audit.py` should check the company's current threshold status before flagging a missing disclosure as an error.
- Related-party transaction materiality for SME-listed entities is ₹50 Cr or 10% of consolidated turnover (whichever is lower) — a different bound than main-board. Use the segment-correct threshold when classifying RPT materiality, not a single hardcoded value.
- SEBI has an open consultation specifically reviewing whether to extend further LODR governance provisions to SME companies. This boundary is under active regulatory review — re-verify the exemption thresholds periodically rather than treating Section 11.1 as permanently fixed.
- **Migration events:** SME companies whose paid-up capital/face value crosses ₹25 Cr become ineligible for the SME segment and must migrate to Main Board. `listingPlatform` is therefore not static per scrip — the pipeline needs a migration-detection check (Tier 0, via exchange announcement) rather than assuming a fixed platform forever.

### 11.2 Concall availability differs sharply by segment

Formal analyst/institutional-investor concalls are common for well-covered mainboard names but rare for SME/Emerge — thin analyst coverage and low institutional interest mean many SME/Emerge companies never hold one.

**Design implication:** Tier 1 concall extraction is *opportunistic* for SME/Emerge, not expected. The primary qualitative source for this segment shifts to the annual report MD&A section and any investor-presentation PDF filed with the exchange. `ConcallOfficialFetcher`'s fallback order should be segment-aware:

```python
def get_fallback_order(listing_platform: str) -> list[str]:
    if listing_platform in ("BSE_SME", "NSE_EMERGE"):
        return ["SCREENER_PDF", "ANNUAL_REPORT_MDA", "BSE_FEED"]  # MD&A promoted, not last resort
    return ["SCREENER_PDF", "BSE_FEED", "YT_SUBTITLE", "AUDIO_TRANSCRIBED", "ANNUAL_REPORT_MDA"]
```

### 11.3 Weight Tier 0 governance signals more heavily for SME/Emerge

The SME segment has historically carried a materially higher incidence of promoter-driven price manipulation and thin-float pump schemes than mainboard — a structural liquidity/oversight difference, not a claim about any specific company. Since SME/Emerge scrips also have less concall/analyst-coverage data to cross-check against, the Tier 0 governance fields (promoter pledge trend, insider transaction direction, auditor change, related-party transaction trend) should carry proportionally more weight in the composite view for this segment, precisely because there's less other information available to contextualize them.

```python
GOVERNANCE_WEIGHT_MULTIPLIER = {
    "BSE_MAIN": 1.0, "NSE_MAIN": 1.0,
    "BSE_SME": 1.4, "NSE_EMERGE": 1.4,
}
```

Don't reduce Tier 0 polling frequency for SME/Emerge just because Tier 1 data is thin — if anything this is the segment where deterministic red-flag detection carries the most relative value.

### 11.4 Source adapter changes

- BSE Main and BSE SME announcements come through the same underlying JSON endpoint family but segmented by market type — pass the segment filter explicitly rather than relying on a default scope.
- NSE Main and NSE Emerge have separate compliance calendars and separate announcement feeds — poll as two distinct `FilingSource` instances, not one NSE source.
- Screener.in/`screener-finance` coverage of SME names has historically been thinner than mainboard. Verify actual coverage against your specific SME/Emerge watchlist before assuming parity; budget for a higher "no data available" rate for this segment and route those cases into the same quarantine-review flow rather than silently returning nulls.

### 11.5 Schema field (already reflected in Section 5.3)

```python
listingPlatform: Literal["BSE_MAIN", "NSE_MAIN", "BSE_SME", "NSE_EMERGE"]
```

Populated at ingestion from exchange metadata, checked for migration events on every Tier 0 run, not set once and forgotten.

---

## 12. Open Items for Next Iteration
