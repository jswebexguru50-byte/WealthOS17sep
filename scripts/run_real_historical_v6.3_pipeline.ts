...
import { evaluateRiskOracle } from '../src/server/services/research/IdealizedRiskOracle.js';
import { AblationEngine, LayerFn } from '../src/server/services/research/AblationEngine.js';
import { bootstrapExpectancy, calculateMetrics } from '../src/server/services/research/StatisticsEngine.js';
import { evaluateGate } from '../src/server/services/research/PromotionGate.js';
import { runResearchPreflight, assertResearchDataComplete, PreflightResult } from '../src/server/services/research/ResearchPreflight.js';
import { attributeTradeRegimes, MarketRegime } from '../src/server/services/research/ResearchRegimeEngine.js';
import { sha256File, sha256String } from '../src/server/services/research/ArtifactHasher.js';
import { evaluateHistoricalOverlay, HistoricalOverlayContext } from '../src/server/services/research/FrozenOverlayAdapter.js';
import type { ResearchSignal, ResearchBar, ExecutionConfig, TradeIdentityLedger, SignalProvenance } from '../src/server/services/research/types.js';
...
const STRATEGY_REGISTRY = [
  ...
  { id: "S2", evaluator: (engine: any, candles: Candle[], sym: string) => engine.evaluateStrategy2(candles, sym) },
  { id: "S3", evaluator: (engine: any, candles: Candle[], sym: string) => engine.evaluateStrategy3(candles, sym) },
  { id: "S4", evaluator: (engine: any, candles: Candle[], sym: string) => engine.evaluateStrategy4(candles, sym) },
  ...
  { id: "S10", evaluator: (engine: any, candles: Candle[], sym: string) => engine.evaluateStrategy10(candles, sym) },
  { id: "S11", evaluator: (engine: any, candles: Candle[], sym: string) => engine.evaluateStrategy11(candles, sym) }
];
  ...
  for (const sym of symbolList) {
    ...
    for (let i = 260; i < candles.length - 1; i++) {
      ...
      for (const reg of STRATEGY_REGISTRY) {
        ...
        const stopLossVal = res?.stopLoss ?? (res as any)?.invalidationStopLoss;

        if (res && res.qualified && stopLossVal && stopLossVal < currentBar.close && stopLossVal > 0) {
          const entry = currentBar.close;
          const stop = Number(stopLossVal.toFixed(2));
          const targetVal = res.target1 ?? (res as any).target;
          if (!targetVal || targetVal <= entry) {
            throw new Error(`DATA_INSUFFICIENT: REQUIRED_FIELD_UNAVAILABLE: Strategy ${reg.id} target1 unavailable on ${sym}`);
          }
          const target = Number(targetVal.toFixed(2));
          const riskPerShare = entry - stop;
          if (riskPerShare <= 0) {
            throw new Error(`DATA_INSUFFICIENT: POSITION_SIZING_UNDEFINED: Invalid risk per share on ${sym}`);
          }
          const targetRisk = 10_000_000 * 0.015; // 1.5% institutional risk policy
          const quantityHint = Math.floor(targetRisk / riskPerShare);
          if (!quantityHint || quantityHint <= 0) {
            throw new Error(`DATA_INSUFFICIENT: POSITION_SIZING_UNDEFINED: Computed quantity 0 on ${sym}`);
          }
      ...
      for (const c of chalTriggers) {
        if (c.res && c.res.qualified && c.res.action === 'BUY' && c.res.stopLoss && c.res.stopLoss < currentBar.close && c.res.stopLoss > 0) {
          const entry = currentBar.close;
          const stop = Number(c.res.stopLoss.toFixed(2));
          const targetVal = c.res.target1 ?? (c.res as any).target;
          if (!targetVal || targetVal <= entry) {
            throw new Error(`DATA_INSUFFICIENT: REQUIRED_FIELD_UNAVAILABLE: Challenger ${c.id} target unavailable on ${sym}`);
          }
          const target = Number(targetVal.toFixed(2));
          const riskPerShare = entry - stop;
          if (riskPerShare <= 0) {
          ...
          const targetRisk = 10_000_000 * 0.015;
          const quantityHint = Math.floor(targetRisk / riskPerShare);
          if (!quantityHint || quantityHint <= 0) {
            throw new Error(`DATA_INSUFFICIENT: POSITION_SIZING_UNDEFINED: Computed quantity 0 on ${sym}`);
          }
